...
- **Song-card text was clipped when tags ran long.** The text panel is a fixed-height box (the cover), but tag chips wrapped onto as many rows as they needed, so on clips with long free-text tags — common on V6 — the stats line and the model badge were pushed below the box and silently cut off. Every row now has a one-line budget: chips that do not fit are hidden whole (a single over-long chip is ellipsised), the title is capped at two lines, and the model and secondary badges moved onto the stats line, after the stats. Worst case measured in Chrome (two-line title, long tags, three secondary badges): the last row ends at 98px of the 112px panel.
- **Warm card requests hung into a 504.** Measured 2026-09-17 on Vercel Edge, on production `main` as well as this branch: the first render of a card answered in ~1s and every repeat request for the same cover ran to the 25s platform limit and returned 504 (a cover URL not yet seen — e.g. the player layout's larger size — worked exactly once, then failed the same way). Cover and avatar fetches no longer go through Next's data cache — the finished SVG is already CDN-cached via `s-maxage` — and the fetch now races a timer, so a hung body read falls back to the placeholder cover instead of failing the route. After the change, repeated requests for the same card all answered 200 in under 1.3s.
- **Custom and uploaded covers were fetched at full size.** `resizeSunoCover` only recognised `cdn2.suno.ai/image_<uuid>.jpeg`, but custom covers live at `<uuid>.jpeg`, `<uuid>_<hash>.jpeg` and `video_upload_…_image.jpeg` on the same host. All 21 such covers sampled on 2026-09-17 honour the same `?width` whitelist (256 → 200, 240 → 403), and they are the heavy originals — up to 3.5 MB, against ~20–50 KB at `?width=256`. Any cover path on `cdn2` that carries a UUID is now resized.
- **The NEW ribbon was clipped to "EW".** Its pulse animation set a CSS `transform`, which *replaces* an SVG element's `transform` attribute rather than composing with it — so mid-pulse the ribbon lost its `translate`, jumped to the card's top-left corner and was cut off by the card edge. Position and animation now sit on separate groups, and the pulse scales about the ribbon's own box (`transform-box: fill-box`), keeping it inside the cover at peak scale in both layouts.
- **Clips no model generated no longer get a `chirp-chirp` badge.** Uploads and studio exports have an empty `major_model_version`, and the fallback badge was printing the placeholder model name instead.
...
### Added
- **Gradient model badges.** Suno sends `text_color_gradient` for V6 (`["FD429C", "FF5126"]` on both schemes). V6-MINI sends six grey stops; any number of stops is kept. It is mapped to `BadgeTheme.gradient` and drawn with `background-clip: text` inside an `@supports` guard, so a renderer without it shows the plain `text_color`.
- **`model_badges.songcard`**, a sibling of `songrow` added alongside the colour removal. It is Suno's artwork-overlay badge (V6 family: translucent `0000004D` background, `display_name` collapsed to `V6`). The card still prefers `songrow`; when it falls back to `songcard` it keeps the text colour and gradient but drops the overlay background and border.
- **Secondary badge chips, off by default.** `SunoSong.secondaryBadges: { key, label }[] | null` — `key` is `icon_key` (else `display_name`) lower-cased, `label` is `display_name` title-cased, because Suno now sends it upper-case (`COVER`, `UPLOAD`, `FULL SONG`, `EXTEND 1` → `{ key: "part", label: "Extend 1" }`). `null` means Suno did not send the key, which is also what a `suno`-prefixed User-Agent gets. New toggle `show_secondary_badges` on `/api/card`, `/api/cards`, the builder, and the Action (`show_secondary_badges` input, service and local modes); chips render beside the model badge in the classic layout.
